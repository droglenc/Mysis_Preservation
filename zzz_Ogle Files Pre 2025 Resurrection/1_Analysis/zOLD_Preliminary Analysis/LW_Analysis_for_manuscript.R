...
plot(c.wt~len,pch=as.numeric(tx),col=as.numeric(stage))      # plot relationship on original scale
legend(x="topleft",legend=levels(tx:stage),col=rep(seq(1,4),3),pch=rep(seq(1,4),each=4))
plot(logwt~loglen,pch=as.numeric(tx),col=as.numeric(stage))  # plot relationship on log-log scale -- not increased variance at low values
legend(x="topleft",legend=levels(tx:stage),col=rep(seq(1,4),3),pch=rep(seq(1,4),each=4))
